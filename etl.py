...
def read_song_data(spark, input_data):
    ...
    song_data = spark.read.json(os.path.join(input_data, 'song_data', '*', '*', '*', '*.json'), schema=songSchema)    
...
def process_song_data(spark, input_data, output_data, song_df):
    ...
    songs_table = song_df.select(song_df.song_id, song_df.title, song_df.artist_id, song_df.year, song_df.duration).dropDuplicates()
    # songs_table.show()
    ...
    artists_table = song_df.select(song_df.artist_id, song_df['artist_name'].alias('name'), song_df['artist_location'].alias('location'), song_df['artist_latitude'].alias('latitude'), song_df['artist_longitude'].alias('longitude')).dropDuplicates()
...
def process_log_data(spark, input_data, output_data, song_df):
    ...
    users_table = log_df.select(log_df.userId, log_df.first_name, log_df.last_name, log_df.gender, log_df.level).dropDuplicates()
    ...
    .withColumn('weekday', F.dayofweek(log_df.start_time)).dropDuplicates()
    ...
    conditions = [log_df.artist == song_df.artist_name, log_df.song == song_df.title]
    ...
    .select(F.row_number().over(Window.partitionBy().orderBy(log_df.sessionId)) \
            ...
            .alias('start_time'), log_df.userId, log_df.level, song_df.song_id, song_df.artist_id, \
            log_df.sessionId, log_df.location, log_df.userAgent) \
    .dropna(how = "any", subset = ["userId", "sessionId"]).dropDuplicates()

    # write songplays table to parquet files partitioned by year and month
    songplays_table.write.mode('overwrite').parquet(os.path.join(output_data, 'songplays/songplays.parquet'))
...
def main():
    ...
    # task paraslleslism: https://knowledge.udacity.com/questions/73278
    sc = spark.sparkContext
    sc._jsc.hadoopConfiguration().set("mapreduce.fileoutputcommitter.algorithm.version", "2")
    
    input_data = "s3a://udacity-dend/"
    output_data = "s3a://jpcsparkifybckt/"
    ...
    print("Elapsed time: ~{:.2f} seconds or ~{:.2f} minutes.").format((end_time - start_time), elapsed_time)
